...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // 1000 HIR with a 250,000,000-block halving interval yields ~500B HIR.
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (block.GetBlockTime() <= pindexPrev->GetBlockTime() - 15 * 60)
        return state.Invalid(ValidationInvalidReason::BLOCK_TIME_PAST, false, REJECT_INVALID, "time-too-old", "block timestamp is too early");
